...
import com.gokhanzopcuk.catchthesonicc.databinding.ActivityEasyBinding
...
class EasyActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        object : CountDownTimer(30000,1000){
            override fun onTick(millisUntilFinished: Long) {
                binding.textView11.text="Time: ${millisUntilFinished/1000}"
            }
            override fun onFinish() {
                binding.textView11.text="Tıme: 0"
                handler.removeCallbacks(runnable)
                for (image in imageArray){
                    image.visibility= View.INVISIBLE
                }
                val alert= AlertDialog.Builder(this@EasyActivity)
                ...
                alert.setNegativeButton("No", DialogInterface.OnClickListener{ DialogInterface, İ->
                    Toast.makeText(this@EasyActivity,"Congratulations!!!",
    ...
    fun increaseScore(view: View){
        score+=1
        binding.textView12.text= "Score:${score} "
    }
}